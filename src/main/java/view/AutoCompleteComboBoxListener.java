 ...
 *
 * @param <T> The type of the list view
 * @author <a href=
    ...
    public AutoCompleteComboBoxListener(ComboBox<T> comboBox) {
        ...
        this.comboBox.setOnKeyReleased(event -> {
            ...
            for (int i = 0; i < items.size(); i++) {
                if (items.get(i) != null && comboBox.getEditor().getText() != null && items.get(i).toString()
                        .toLowerCase().startsWith(comboBox.getEditor().getText().toLowerCase())) {
                    try {
                        comboBox.getEditor()
                                .setText(sb.toString() + items.get(i).toString().substring(sb.toString().length()));
